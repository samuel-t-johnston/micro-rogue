...
// (port 'branch1') leading to a two-floor side branch (branch 1) — a large BSP floor, then a walker
// cave floor beneath it. Named ports map to a stair's port; most stairs use their direction
// ('up'/'down'), but a second same-direction stair takes a distinct port (see data/maps/floor-1-a.js
// and docs/howto/dungeon-layout.md). `dir: 'bidi'` means the stairs can be climbed both ways. The
// general model (exit/enter capabilities, contract validation, a transit-map visualizer) is designed
// in docs/design/dungeon-planner.md but not built yet.
export default {
  ...
  nodes: [
    ...
    // Branch 1: a large BSP floor reached from floor-1's second (start-room) down-stair, then a walker
    // cave floor below it.
