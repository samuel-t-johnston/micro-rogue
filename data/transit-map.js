...
// (port 'branch1') leading to a three-floor side branch (branch 1) — a large BSP floor, then a walker
// cave floor, then a cellular-automata cave floor, each deeper than the last. Named ports map to a
// stair's port; most stairs use their direction ('up'/'down'), but a second same-direction stair takes
// a distinct port (see data/maps/floor-1-a.js and docs/howto/dungeon-layout.md). `dir: 'bidi'` means
// the stairs can be climbed both ways. The general model (exit/enter capabilities, contract
// validation, a transit-map visualizer) is designed in docs/design/dungeon-planner.md but not built yet.
export default {
  ...
  nodes: [
    ...
    // floor, then a cellular-automata cave floor.
