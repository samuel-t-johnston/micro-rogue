import { initializeWorldAsync, placeRandomItems, placeRandomFurniture } from './world.js';
...
export function movePlayer(dx, dy, gameState, gameDisplay, choiceModeManager = null) {
  ...
  if (!gameState.currentLevel.isPassible(newPos.x, newPos.y)) {
    ...
    if (furniture) {
      addMessage(`You can't move through the ${furniture.getName()}!`, gameState, gameState.player);
    } else {
  ...
  if (itemAtPosition) {
    const itemName = gameState.itemsData[itemAtPosition.itemId]?.name || 'Unknown Item';
    addMessage(`You see a ${itemName} here.`, gameState, gameState.player);
  }

  // Check if there's furniture at the new position
  const furnitureAtPosition = gameState.currentLevel.getFurnitureAt(newPos.x, newPos.y);
  if (furnitureAtPosition) {
    ...
    if (furnitureAtPosition.isContainer() && furnitureAtPosition.state === 'open') {
...
export async function initGame() {
  ...
  placeRandomFurniture(gameState.furnitureData, gameState.currentLevel, gameState.itemsData);
  ...
  addMessage(`Your character has ${gameState.player.maxHp} max HP.`, gameState, gameState.player);
  addMessage('You are in a simple room. Explore with WASD!', gameState, gameState.player);
  addMessage('You see some items ($) and furniture scattered around the room.', gameState, gameState.player);
...
export function pickUpItem(gameState, gameDisplay, choiceModeManager = null) {
  ...
  if (!gameState.player.canAddToInventory()) {
    addMessage("Can't pick anything else up. Inventory is full.", gameState, gameState.player);
    return;
  }
  ...
  if (itemAtPosition) {
    ...
    const furnitureAtPosition = gameState.currentLevel.getFurnitureAt(playerPos.x, playerPos.y);
    
    if (furnitureAtPosition && furnitureAtPosition.isContainer() && furnitureAtPosition.state === 'open') {
      const containerItems = furnitureAtPosition.getContainerItems();
      ...
      if (containerItems.length > 0) {
        ...
        if (item) {
          ...
          if (addedItem) {
            addMessage(`Took ${item.name} from the ${furnitureAtPosition.getName()}.`, gameState, gameState.player);
            render(gameState, gameDisplay);
            updateUI(gameState, gameState.player, choiceModeManager);
          } else {
        ...
        addMessage(`The ${furnitureAtPosition.getName()} is empty.`, gameState, gameState.player);
      }
    } else {
...
export function pickUpItemByIndex(index, gameState, gameDisplay, choiceModeManager = null) {
  ...
  if (addedItem) {
    ...
    if (selectedItem.source === 'ground') {
      ...
      addMessage(`Picked up ${selectedItem.name}!`, gameState, gameState.player);
    } else if (selectedItem.source === 'container') {
      const furnitureAtPosition = gameState.currentLevel.getFurnitureAt(gameState.getPlayerPosition().x, gameState.getPlayerPosition().y);
      if (furnitureAtPosition) {
        furnitureAtPosition.removeItemFromContainer(selectedItem.containerIndex);
        addMessage(`Took ${selectedItem.name} from the ${selectedItem.containerName}.`, gameState, gameState.player);
      }
    }
...
export function useFurniture(dx, dy, gameState, gameDisplay, onUIUpdate = null) {
  const playerPos = gameState.getPlayerPosition();
  ...
  if (!furniture.isUsable()) {
    addMessage(`You can't use the ${furniture.getName()}.`, gameState, gameState.player);
    if (onUIUpdate) onUIUpdate();
    return false;
  }
  ...
  if (furnitureDef.usable.action === 'toggle_state') {
    if (furniture.toggleState()) {
      ...
      if (furniture.isContainer()) {
        ...
        } else if (newState === 'open' && furniture.getContainerItems().length === 0) {
          message += ' It is empty.';
        }
      }
      ...
      addMessage(`You can't toggle the ${furniture.getName()}.`, gameState, gameState.player);
      if (onUIUpdate) onUIUpdate();
      return false;
    }
  }

  addMessage(`You don't know how to use the ${furniture.getName()}.`, gameState, gameState.player);
  if (onUIUpdate) onUIUpdate();
  return false;
}
