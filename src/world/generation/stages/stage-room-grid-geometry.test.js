...
describe('room-grid geometry stage', () => {
  ...
  it('derives each zone rect from its cells in tile space', () => {
    ...
    for (const z of zones) {
      if (z.cells.length === 1) {
        ...
        const spans2x1 = (z.rect.w === cs * 2 && z.rect.h === cs) || (z.rect.w === cs && z.rect.h === cs * 2);
  ...
  it('honours cols/rows/cellSize parameters', () => {
    const { 'level:zones': zones, 'level:grid': grid } = generate(123, { cols: 4, rows: 4, cellSize: 8 });
  ...
  it('keeps the graph connected across multiple deletes, with no isolated zone', () => {
    for (let seed = 1; seed <= 20; seed++) {
      ...
      for (const [a, b] of adjacency) { degree.set(a, degree.get(a) + 1); degree.set(b, degree.get(b) + 1); }
  ...
  it('is deterministic for a given seed', () => {
    ...
    expect(generate(777, { deletes: 2, merges: 2 })).toEqual(generate(777, { deletes: 2, merges: 2 }));
  });
