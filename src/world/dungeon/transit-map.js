// Pure lookups over a transit-map data object (see data/transit-map.js). The map is plain data so the
// resolver, the dungeon runtime (level-manager.js), and any future visualizer all read it the same
// way. An edge is `{ a: [node, port], b: [node, port], dir: 'bidi' | 'uni' }`; a 'uni' edge is only
// traversable from `a` to `b` (e.g. a pit you fall down but can't climb back up).
