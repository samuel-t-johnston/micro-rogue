/**
 * Collects the transitive closure of entities reachable from `roots`, following the entity references
 * that bind an entity graph together: `inventory.items` and worn `wearsEquipment.slots` (a container
 * like a chest carries an `inventory`, so its contents are reached the same way).
 *
 ...
 * needs no registry. Returns a Set of entities including the roots; a `found` guard makes cycles and
