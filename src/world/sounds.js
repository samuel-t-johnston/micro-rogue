...
export function emitSound(registry, level, { sourceId = null, x, y, volume = 0, language = null, message = null, lifespan = 2 }) {
  const sound = registry.createEntity();
  registry.addComponent(sound, 'position', components.position(x, y));
  registry.addComponent(sound, 'sound', components.sound({ sourceId, volume, language, message }));
