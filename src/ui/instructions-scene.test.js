...
function makeCtx() {
  ...
  return new Proxy({}, {
    get: (_, key) => {
      ...
      return (key === 'font' || key === 'fillStyle' || key === 'strokeStyle' || key === 'lineWidth' ||
              key === 'textAlign' || key === 'textBaseline' || key === 'globalAlpha') ? '' : noop;
    },
    set: () => true,
  });
}
...
describe('instructions scene', () => {
  ...
  it('Enter fires onContinue', () => {
    let cont = 0;
    const scene = createInstructionsScene({ theme, getViewport, onContinue: () => { cont++; } });
