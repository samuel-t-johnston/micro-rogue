...
export function createActionMenu({ theme, getViewport, title, actions, onSelect }) {
  function layout() {
    ...
    const panelH = HEADER_H + PADDING + actions.length * BUTTON_H + (actions.length - 1) * BUTTON_GAP + PADDING;
  ...
  return {
    render(ctx) {
      ...
      drawText(ctx, title, panel.x + panel.w / 2, panel.y + HEADER_H / 2, {
        color: theme.text, size: 15, weight: '600', align: 'center', baseline: 'middle',
      });
    ...
    handleInput(event) {
      ...
      if (event.type === 'pointerdown') {
        ...
        if (!hitTest(panel, event.x, event.y)) { onSelect(null); return true; }
