...
// A horizontal segmented control: `options` (string labels) drawn as adjacent segments with the
...
// A square checkbox at (x, y) with its `label` to the right, vertically centered on the box. Checked
// state is a filled inset square (color, not glyph, so it survives any font). The caller owns the
// clickable region (typically the box + label row) and hit-tests it itself.
