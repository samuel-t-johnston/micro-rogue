...
_Landed (first cut):_ `src/render/vignette.js` — an ambient singleton (sibling of `animations`) with two modes: timed edge-glow **pulses** (`trigger`, parameterized by `color`/`pulses`/`pulseLength` + optional `maxAlpha`) and keyed **sustained** overlays (`setSustained(key, spec)`) held steady while a state persists. It draws last in the game-scene frame, above the UI, so a warning reads even over an open menu. Uses so far: the gold single-pulse **level-up** flourish (from `level-up.js`) and a thin red **starving** overlay held while `hungerStatus` is `'starving'` (from game-scene), paired with a red `(Hungry)`/`(Starving!)` label on the HUD's HP line. Pulses honor the reduced-motion kill switch; the sustained overlay is a static tint (the accessible fallback form) and stays on. A red two-pulse damage "heartbeat" and the other persistent-status vignettes above are the next users; the settings toggle is still to come.
