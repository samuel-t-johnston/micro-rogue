...
- **One verb can carry an intent.** A container interaction is two-faceted: the default takes items out, while the resolver's secondary `Place items` row sets `mode: 'store'` on the same `interact` action to put items in. The handler branches on `action.mode` and reuses the one multi-select dialog (relabelled `Place`) rather than a second action type — so taking and placing share adjacency, target resolution, and the "moved ≥1 item → turn consumed" rule. A plain tap never sets a mode, so it always takes/opens.
