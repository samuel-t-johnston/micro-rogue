...
- **Use color for branding, not accessibility.** A shared color scheme makes the application feel cohesive, but colors by themselves are a poor channel for communicating information, because a large percentage of humans are colorblind. Try to include a second information channel along with color — shape, text, etc. For example: when debug is enabled, the log shows debug lines in red, but also {encloses them in curly braces}. You may have a red potion and green potion in the game, but the "look at" action and inventory state the color in text. (Different bottle shapes in your potion sprites might be a good idea too.)
