...
export class LevelLoader {
  ...
  parseLevel(levelData) {
    ...
    const furniture = this.parseFurniture(
      level_data.grid,
      level_data.width,
      level_data.height
    );
  ...
  parseGrid(gridArray, width, height) {
    if (gridArray.length !== height) {
      throw new Error(
        `Grid height mismatch: expected ${height}, got ${gridArray.length}`
      );
    }
    ...
    for (let y = 0; y < height; y++) {
      ...
      if (row.length !== width) {
        throw new Error(
          `Grid row ${y} width mismatch: expected ${width}, got ${row.length}`
        );
      }
  ...
  validateLevelStructure(levelData) {
    ...
    if (
      !Array.isArray(level_data.player_start) ||
      level_data.player_start.length !== 2
    ) {
