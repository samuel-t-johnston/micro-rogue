...
async function initializeWorldV2() {
  try {
    ...
    const levelData = await levelLoader.loadLevel('./levels/1-test-dungeon.json');
...
export function placeRandomFurniture(furnitureData, dungeonLevel, itemsData) {
  ...
  if (window.loadedLevelData && window.loadedLevelData.furniture) {
    placeLoadedFurniture(furnitureData, dungeonLevel, itemsData, window.loadedLevelData);
    return;
  }
}

// Place furniture from loaded level data
function placeLoadedFurniture(furnitureData, dungeonLevel, itemsData, levelData) {
